...
function aboutMessage(d) {
  const lines = [LABEL.about];
  lines.push(d.news
    ? 'Check the news claim against the link, fill anything in brackets, delete the NEWS SOURCE block, then copy the draft into LinkedIn.'
    : 'Fill anything in brackets, then copy the draft above into LinkedIn.');

  lines.push('');
  if (d.news) lines.push(`News angle: "${d.news.headline}" (${d.news.source}, ${d.news.date}) - found by searching Google News for "${d.newsPhrase || 'the draft topic'}".`);
  else if (d.newsCandidates?.length) lines.push(`No news angle: I searched Google News for "${d.newsPhrase}", but nothing fitted naturally (top result: "${d.newsCandidates[0].headline}", ${d.newsCandidates[0].source}).`);
  else lines.push(`No news angle: Google News had nothing recent for "${d.newsPhrase || 'this topic'}".`);

  const fills = (d.body.match(/\[[^\]]+\]/g) || []);
...
async function handleNote(chatId, msgId, note, { force = false, voice = false } = {}) {
  ...
  if (!force && !t.passed) {
    await telegram.sendMessage(chatId,
      `${LABEL.skip} (${t.score}/10): ${t.reason}\n\nNote: "${quoted(note)}"\n\nReply "draft" to this message to draft it anyway.`,
      { replyTo: msgId });
    return;
  }
  await telegram.sendMessage(chatId,
    `${LABEL.worth} (${t.score}/10): ${t.reason}\nDrafting now and checking Google News for a current angle - about a minute.${heard}`,
    { replyTo: msgId });
...
async function handleRedraft(chatId, msgId, previousPost, feedback) {
  await telegram.sendMessage(chatId, `${LABEL.redraft} - about a minute.`, { replyTo: msgId });
  await telegram.typing(chatId);
